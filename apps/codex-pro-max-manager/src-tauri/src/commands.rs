...
use codex_pro_max_core::install::SILENT_BINARY;
use codex_pro_max_core::script_market::{self, MarketScript, ScriptMarketManifest};
use codex_pro_max_core::settings::{BackendSettings, RelayProfile, SettingsStore};
use codex_pro_max_core::status::{LaunchStatus, StatusStore};
use codex_pro_max_core::user_scripts::UserScriptManager;
use serde::Serialize;
...
pub struct CcsProvidersPayload {
    pub db_path: String,
    pub providers: Vec<codex_pro_max_core::ccs_import::CcsProviderImport>,
}
...
pub struct ContextEntriesPayload {
    pub settings: BackendSettings,
    pub entries: codex_pro_max_core::relay_config::CodexContextEntries,
}
...
pub fn backend_version() -> CommandResult<VersionPayload> {
    ok(
        "后端版本已读取。",
        VersionPayload {
            version: codex_pro_max_core::version::VERSION.to_string(),
        },
    )
}
    ...
    let Ok((codex_app_path, entrypoints, latest_launch)) = payload else {
        return failed(
            ...
            OverviewPayload {
                ...
                latest_launch: None,
                current_version: codex_pro_max_core::version::VERSION.to_string(),
                update_status: "not_checked".to_string(),
                settings_path: codex_pro_max_core::paths::default_settings_path()
                    .to_string_lossy()
                    .to_string(),
                logs_path: codex_pro_max_core::paths::default_diagnostic_log_path()
                    .to_string_lossy()
                    .to_string(),
            },
        );
    ...
    ok(
        ...
        OverviewPayload {
            codex_version: codex_app_path
                .as_deref()
                .and_then(codex_pro_max_core::app_paths::codex_app_version),
...
#[tauri::command]
pub fn launch_codex_pro_max(request: LaunchRequest) -> CommandResult<Value> {
    spawn_codex_pro_max_launch(request, "启动任务已在后台开始，可稍后查看概览状态。")
}

#[tauri::command]
pub fn restart_codex_pro_max(request: LaunchRequest) -> CommandResult<Value> {
    codex_pro_max_core::watcher::stop_launcher_processes();
    codex_pro_max_core::watcher::stop_codex_processes();
...
fn spawn_silent_launcher(request: &LaunchRequest) -> anyhow::Result<()> {
    let launcher = codex_pro_max_core::install::companion_binary_path(SILENT_BINARY);
...
pub fn save_settings(settings: BackendSettings) -> CommandResult<SettingsPayload> {
    ...
    if settings.ccs_link_enabled {
        if let Err(error) = codex_pro_max_core::ccs_import::write_linked_profiles_to_default_db(
        ...
        if !active.linked_ccs_provider_id.trim().is_empty() {
            if let Err(error) =
                codex_pro_max_core::ccs_import::set_current_codex_provider_in_default_db(
...
pub fn load_ccs_providers() -> CommandResult<CcsProvidersPayload> {
    let db_path = codex_pro_max_core::ccs_import::default_ccs_db_path();
    match codex_pro_max_core::ccs_import::list_codex_providers_from_db(&db_path) {
        Ok(providers) => ok(
...
pub fn import_ccs_providers() -> CommandResult<SettingsPayload> {
    ...
    let synced = match codex_pro_max_core::ccs_import::list_codex_providers_from_default_db() {
...
fn normalize_settings_before_save(mut settings: BackendSettings) -> BackendSettings {
    if let Some(path) =
        codex_pro_max_core::app_paths::normalize_codex_app_path(Path::new(&settings.codex_app_path))
    {
        settings.codex_app_path = path.to_string_lossy().to_string();
    }
    settings.relay_common_config_contents =
        codex_pro_max_core::relay_config::sanitize_common_config_contents(
            &settings.relay_common_config_contents,
        );
    ...
    for profile in &mut settings.relay_profiles {
        if let Err(error) =
            codex_pro_max_core::relay_config::normalize_relay_profile_for_storage(profile)
        {
            log_manager_event(
    ...
    if !common_config.trim().is_empty() {
        for profile in &mut settings.relay_profiles {
            ...
            match codex_pro_max_core::relay_config::strip_common_config_from_config(
...
fn settings_with_live_ccs_profiles(mut settings: BackendSettings) -> BackendSettings {
    ...
    remove_linked_ccs_profiles_for_local_storage(&mut settings);
    if let Err(error) = codex_pro_max_core::ccs_import::sync_linked_profiles_from_default_db(
...
fn remove_linked_ccs_profiles_for_local_storage(settings: &mut BackendSettings) {
        ...
        .any(|profile| profile.id == settings.active_relay_id)
    {
        settings.active_relay_id = settings
            ...
            .unwrap_or_else(codex_pro_max_core::settings::default_active_relay_id);
    }
}
...
fn relay_join_config_sections(sections: &[&str]) -> String {
    ...
    if sections.is_empty() {
        ...
        codex_pro_max_core::relay_config::normalize_config_text(&format!(
            "{}\n",
            sections.join("\n\n")
        ))
    }
...
pub fn repair_backend() -> CommandResult<SettingsPayload> {
    ...
    let message = match codex_pro_max_core::cli_wrapper::ensure_cli_wrapper(&settings) {
...
pub async fn check_update() -> CommandResult<Value> {
    match codex_pro_max_core::update::check_for_update(codex_pro_max_core::version::VERSION).await {
        Ok(update) => {
        ...
        Err(error) => failed(
            &format!("检查更新失败：{error}"),
            json!({
                "currentVersion": codex_pro_max_core::version::VERSION,
...
pub async fn perform_update(
    release: Option<codex_pro_max_core::update::Release>,
) -> CommandResult<Value> {
    ...
    let download_dir = codex_pro_max_core::paths::default_app_state_dir().join("updates");
    match codex_pro_max_core::update::perform_update(&release, &download_dir).await {
        Ok(result) => ok(
...
pub fn uninstall_watcher() -> CommandResult<WatcherPayload> {
    match codex_pro_max_core::watcher::uninstall_watcher() {
...
pub fn enable_watcher() -> CommandResult<WatcherPayload> {
    match codex_pro_max_core::watcher::enable_watcher() {
...
pub fn disable_watcher() -> CommandResult<WatcherPayload> {
    match codex_pro_max_core::watcher::disable_watcher() {
...
pub fn read_latest_logs(request: LogRequest) -> CommandResult<LogsPayload> {
    let path = codex_pro_max_core::paths::default_diagnostic_log_path();
...
pub fn relay_status() -> CommandResult<RelayPayload> {
    let status = codex_pro_max_core::relay_config::default_relay_status();
...
pub fn read_relay_files() -> CommandResult<RelayFilesPayload> {
    let home = codex_pro_max_core::relay_config::default_codex_home_dir();
...
pub fn write_diagnostic_event(event: String, detail: Value) -> CommandResult<Value> {
    ...
    match codex_pro_max_core::diagnostic_log::append_diagnostic_log(&event, detail) {
        Ok(()) => ok("诊断日志已写入。", json!({})),
...
pub fn backfill_relay_profile_from_live(
    ...
    match codex_pro_max_core::relay_config::backfill_relay_profile_from_home_with_common(
        &home,
        profile,
...
pub fn list_context_entries(
    ...
    match codex_pro_max_core::relay_config::list_context_entries_from_common_config(
...
pub fn read_live_context_entries() -> CommandResult<LiveContextEntriesPayload> {
    ...
    match codex_pro_max_core::relay_config::list_context_entries_from_common_config(&config) {
        Ok(entries) => ok(
            "live 工具与插件已读取。",
...
pub fn upsert_context_entry(request: ContextEntryRequest) -> CommandResult<ContextEntriesPayload> {
    let mut settings = request.settings;
    match codex_pro_max_core::relay_config::upsert_context_entry_in_common_config(
        &settings.relay_context_config_contents,
        &request.kind,
        &request.id,
        &request.toml_body,
...
pub fn sync_live_context_entries(
    ...
    let updated_config = match codex_pro_max_core::relay_config::sync_live_config_context_entries(
        &current_config,
        &request.settings.relay_context_config_contents,
    ) {
        Ok(config) => config,
...
pub fn delete_context_entry(request: ContextDeleteRequest) -> CommandResult<ContextEntriesPayload> {
    let mut settings = request.settings;
    match codex_pro_max_core::relay_config::delete_context_entry_from_common_config(
        &settings.relay_context_config_contents,
        &request.kind,
        &request.id,
    ) {
...
pub async fn test_relay_profile(profile: RelayProfile) -> CommandResult<RelayProfileTestPayload> {
    ...
    match codex_pro_max_core::relay_config::test_relay_profile(&profile, test_model).await {
        Ok(result) => {
...
pub async fn fetch_relay_profile_models(
    ...
    match codex_pro_max_core::model_catalog::fetch_relay_profile_model_ids(&profile).await {
...
pub fn apply_relay_injection() -> CommandResult<RelayPayload> {
    ...
    if relay.config_contents.trim().is_empty() == false
        && relay.relay_mode == codex_pro_max_core::settings::RelayMode::Official
        && relay.official_mix_api_key
    {
        return match codex_pro_max_core::relay_config::apply_relay_profile_config_to_home_with_context(
            &home,
            &relay,
    ...
    if relay_has_complete_files(&relay) {
        return match codex_pro_max_core::relay_config::apply_relay_profile_to_home_with_switch_rules(
            &home,
            &relay,
            &relay_combined_common_config(&settings),
        ) {
    ...
    let auth = codex_pro_max_core::relay_config::chatgpt_auth_status_from_home(&home);
    ...
    match codex_pro_max_core::relay_config::apply_relay_config_to_home_with_protocol(
        &home,
        &relay.base_url,
        &relay.api_key,
        relay.protocol,
        codex_pro_max_core::protocol_proxy::DEFAULT_PROTOCOL_PROXY_PORT,
    ) {
        Ok(result) => {
            let status = codex_pro_max_core::relay_config::relay_status_from_home(&home);
            log_relay_apply_result(
...
pub fn apply_pure_api_injection() -> CommandResult<RelayPayload> {
    ...
    match codex_pro_max_core::relay_config::apply_pure_api_config_to_home_with_protocol(
        &home,
        &relay.base_url,
        &relay.api_key,
        relay.protocol,
...
pub fn clear_relay_injection() -> CommandResult<RelayPayload> {
    ...
    let auth_contents = (relay.relay_mode == codex_pro_max_core::settings::RelayMode::Official
        && !relay.official_mix_api_key
...
fn relay_has_complete_files(relay: &codex_pro_max_core::settings::RelayProfile) -> bool {
...
fn log_relay_apply_request(
    ...
    relay: &codex_pro_max_core::settings::RelayProfile,
) {
    let _ = codex_pro_max_core::diagnostic_log::append_diagnostic_log(
        event,
        json!({
...
fn log_relay_apply_result(
    event: &str,
    relay: &codex_pro_max_core::settings::RelayProfile,
    status: &codex_pro_max_core::relay_config::RelayStatus,
...
fn log_manager_event(event: &str, detail: Value) {
    let _ = codex_pro_max_core::diagnostic_log::append_diagnostic_log(event, detail);
}
...
fn refresh_cli_wrapper_after_settings_save(settings: &BackendSettings) -> String {
    match codex_pro_max_core::cli_wrapper::ensure_cli_wrapper(settings) {
        Ok(Some(install)) => format!(
            " 命令包装器已更新：{}。",
            install.real_codex.to_string_lossy()
        ),
...
fn empty_context_entries() -> codex_pro_max_core::relay_config::CodexContextEntries {
    codex_pro_max_core::relay_config::CodexContextEntries {
        mcp_servers: Vec::new(),
        skills: Vec::new(),
        plugins: Vec::new(),
    }
...
fn open_url(url: &str) -> anyhow::Result<()> {
    #[cfg(windows)]
    {
        codex_pro_max_core::windows_open_url(url)
    }
    #[cfg(not(windows))]
    {
...
fn settings_payload_value() -> Result<SettingsPayload, (anyhow::Error, SettingsPayload)> {
    let store = SettingsStore::default();
    let settings_path = codex_pro_max_core::paths::default_settings_path()
        .to_string_lossy()
        .to_string();
    match store.load() {
...
fn user_scripts_config_dir() -> PathBuf {
    if cfg!(windows) {
        if let Some(roaming) = std::env::var_os("APPDATA") {
            return PathBuf::from(roaming).join("codex-pro-max");
        }
    }
...
fn diagnostics_report() -> String {
    ...
    serde_json::to_string_pretty(&json!({
        ...
        "version": codex_pro_max_core::version::VERSION,
        "overview": overview.payload,
        "settings": settings,
        "logs": {
            "diagnosticLogPath": codex_pro_max_core::paths::default_diagnostic_log_path(),
            "latestStatusPath": codex_pro_max_core::paths::default_latest_status_path()
        },
        "platform": {
...
fn load_overview_payload() -> (
    ...
    let settings = SettingsStore::default().load().unwrap_or_default();
    (
        codex_pro_max_core::app_paths::resolve_codex_app_dir_with_saved(
            None,
...
fn watcher_payload() -> WatcherPayload {
    let flag = codex_pro_max_core::watcher::default_watcher_disabled_flag();
    WatcherPayload {
        enabled: !flag.exists(),
...
mod tests {
    ...
    fn startup_options_honors_show_update_argument() {
        assert!(should_show_update(
            ["codex-pro-max-manager.exe", "--show-update"],
            None
        ));
    }
    ...
    fn relay_payload_does_not_expose_token_text() {
        let payload = relay_payload(
            codex_pro_max_core::relay_config::RelayStatus {
                authenticated: true,
    ...
    fn apply_relay_profile_to_home_with_switch_rules_normalizes_custom_provider_id_to_stable_bucket()
     {
        ...
        let profile = RelayProfile {
            relay_mode: codex_pro_max_core::settings::RelayMode::PureApi,
            protocol: codex_pro_max_core::settings::RelayProtocol::Responses,
        ...
        codex_pro_max_core::relay_config::apply_relay_profile_to_home_with_switch_rules(
            temp.path(),
            &profile,
            "",
        )
    ...
    fn normalize_settings_before_save_preserves_official_profile_auth() {
        let settings = BackendSettings {
            relay_profiles: vec![RelayProfile {
                relay_mode: codex_pro_max_core::settings::RelayMode::Official,
