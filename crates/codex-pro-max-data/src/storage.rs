...
use codex_pro_max_core::models::{DeleteResult, DeleteStatus, SessionRef};
    ...
    pub fn find_archived_thread_by_title(&self, title: &str) -> Option<SessionRef> {
        ...
        let mut stmt = db
            ...
            "SELECT id, title FROM threads
                 WHERE archived = 1 AND title = ?1
                 ORDER BY archived_at DESC LIMIT 1",
            )
            .ok()?;
        let mut rows = stmt.query([title]).ok()?;
    ...
    fn delete_codex_thread(
        ...
        for file in file_backups {
            if let Some(path) = file.get("path").and_then(Value::as_str) {
                if let Err(err) = safe_trash_file(Path::new(path), &backup_path) {
...
fn safe_trash_file(path: &Path, backup_path: &Path) -> std::io::Result<()> {
    if !path.exists() {
        return Ok(());
    }
    let trash_root = backup_path
        .parent()
    ...
    let mut target = trash_root.join(file_name);
    let mut suffix = 1usize;
    while target.exists() {
        target = trash_root.join(format!("{suffix}-{file_name}"));
        suffix += 1;
    }
    match fs::rename(path, &target) {
        ...
        Err(rename_error) => {
            fs::copy(path, &target)?;
            fs::remove_file(path).or(Err(rename_error))
        }
    }
}
