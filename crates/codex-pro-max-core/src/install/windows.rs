...
const UNINSTALL_SUBKEY: &str = r"Software\Microsoft\Windows\CurrentVersion\Uninstall\CodexProMax";
const LEGACY_UNINSTALL_SUBKEY: &str =
    r"Software\Microsoft\Windows\CurrentVersion\Uninstall\codex-pro-max";
...
pub fn build_windows_entrypoint_plan(options: &InstallOptions) -> WindowsEntrypointPlan {
    ...
    WindowsEntrypointPlan {
        ...
        uninstall_key: "CodexProMax".to_string(),
        legacy_uninstall_key: "codex-pro-max".to_string(),
...
fn default_icon_path() -> PathBuf {
    std::env::current_exe()
        ...
        .map(|path| path.join("codex-pro-max.ico"))
        .unwrap_or_else(|| PathBuf::from("codex-pro-max.ico"))
}
