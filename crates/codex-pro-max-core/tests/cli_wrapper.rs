...
use codex_pro_max_core::cli_wrapper::{
...
fn wrapper_source_embeds_absolute_real_codex_path() {
    ...
    let source = build_wrapper_source(
        ...
        &PathBuf::from(r"C:\Users\me\.codex-pro-max-cli"),
        &settings,
    );
...
#[test]
fn wrapper_dir_uses_roaming_codex_pro_max_plus() {
    assert_eq!(
        wrapper_dir_from_roaming(&PathBuf::from(r"C:\Users\me\AppData\Roaming")),
        PathBuf::from(r"C:\Users\me\AppData\Roaming\codex-pro-max")
    );
}
...
fn repair_refreshes_when_wrapper_already_exists_even_if_setting_is_disabled() {
    let temp = tempfile::tempdir().unwrap();
    let wrapper_dir = temp.path().join("codex-pro-max");
