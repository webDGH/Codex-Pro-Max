...
use codex_pro_max_core::models::{DeleteResult, DeleteStatus, ExportResult, ExportStatus, SessionRef};
use codex_pro_max_core::routes::{
    BridgeContext, BridgeDataService, BridgeRuntimeService, BridgeSettingsService,
    CoreRuntimeService, handle_bridge_request,
};
use codex_pro_max_core::settings::BackendSettings;
use codex_pro_max_core::status::StatusStore;
...
async fn runtime_status_devtools_repair_and_ads_routes_are_dispatched() {
    ...
    assert_eq!(
        ...
        json!({"status": "ok", "message": "后端已修复", "version": codex_pro_max_core::version::VERSION})
    );
    assert_eq!(
...
async fn user_script_manager_deletes_market_script_metadata_and_rejects_builtin_delete() {
    ...
    let script = codex_pro_max_core::script_market::MarketScript {
        id: "demo".to_string(),
        name: "Demo".to_string(),
    ...
    codex_pro_max_core::script_market::install_market_script_content(
        &manager,
        &script,
        b"window.demo = true;",
    )
...
async fn bridge_backend_status_writes_diagnostic_log() {
    ...
    codex_pro_max_core::diagnostic_log::set_diagnostic_log_path_for_tests(Some(log_path.clone()));
    let ctx = BridgeContext::core(Arc::new(CoreRuntimeService::new(
        9229,
        StatusStore::default(),
    )));
    ...
    codex_pro_max_core::diagnostic_log::set_diagnostic_log_path_for_tests(None);
}
...
fn script_market_manifest_filters_invalid_entries() {
    ...
    let manifest = codex_pro_max_core::script_market::parse_market_manifest(raw).unwrap();
...
fn user_script_inventory_includes_market_metadata() {
    ...
    manager
        .record_market_install(&codex_pro_max_core::script_market::MarketScript {
            id: "demo".to_string(),
            name: "Demo".to_string(),
            description: "Useful demo".to_string(),
            version: "1.0.0".to_string(),
            author: "shgkz".to_string(),
...
fn install_market_script_ignores_checksum_mismatch_and_replaces_existing_file() {
    ...
    codex_pro_max_core::script_market::install_market_script_content(&manager, &script, b"new")
        .unwrap();
