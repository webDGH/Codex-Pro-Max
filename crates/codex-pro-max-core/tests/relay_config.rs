use codex_pro_max_core::relay_config::{
...
use codex_pro_max_core::settings::{RelayContextSelection, RelayMode, RelayProfile, RelayProtocol};
...
fn apply_chat_protocol_relay_points_codex_to_local_responses_proxy() {
    ...
    let result = codex_pro_max_core::relay_config::apply_relay_config_to_home_with_protocol(
        temp.path(),
    ...
    assert!(updated.contains(r#"experimental_bearer_token = "sk-test-redacted""#));
    assert!(!updated.contains("codex_pro_max_chat_base_url"));
}
...
"#
    ...
    assert!(!profile.config_contents.contains("codex_pro_max_chat_base_url"));
    assert!(
        profile
            .config_contents
    ...
    assert!(!live.contains("codex_pro_max_chat_base_url"));
...
fn apply_relay_files_with_context_selection_writes_all_global_context() {
    ...
    codex_pro_max_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "custom"
[model_providers.custom]
name = "custom"
...
fn apply_relay_files_with_context_rejects_invalid_context_token_values() {
    ...
    let error = codex_pro_max_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "custom""#,
        r#"{"OPENAI_API_KEY":"sk-new"}"#,
        "",
